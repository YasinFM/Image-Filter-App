...
void reflect_horizontal1(unsigned char *image, int width, int height, int channels) {
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            image[(j * width + i) * channels] = image[(j*width+(width-1-i))*channels];
            image[(j * width + i) * channels + 1] = image[(j*width+(width-1-i))*channels + 1];
            image[(j * width + i) * channels + 2] = image[(j*width+(width-1-i))*channels + 2];
        }
    }
}
...
void reflect_vertical1(unsigned char *image, int width, int height, int channels) {
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            image[(i * width + j) * channels] = image[((height - i - 1) * width + j) * channels];
            image[(i * width + j) * channels + 1] = image[((height - i - 1) * width + j) * channels + 1];
            image[(i * width + j) * channels + 2] = image[((height - i - 1) * width + j) * channels + 2];
        }
    }
}
...
int main() {
    ...
    printf("13. Rotate (Only for bmp)\n");
    printf("14. Mirror Vertical (Only for jpg and png)\n");
    printf("15. Mirror Horizontal (Only for jpg and png)\n");

    int filter = 0;
    while (!(filter > 0 && filter < 16)) {
        scanf("%d", &filter);
        if (!(filter > 0 && filter < 16)) printf("Enter a filter: ");
    }
    ...
    if (filter == 1) {
        ...
        reflect_vertical1(image, width, height, channels);
        if (format == 1 || format == 4)
            stbi_write_jpg(filename_out, width, height, channels, image, 100);
        else if (format == 2)
            stbi_write_png(filename_out, width, height, channels, image, 0);
    } else if (filter == 15) {
        reflect_horizontal1(image, width, height, channels);
        if (format == 1 || format == 4)
            stbi_write_jpg(filename_out, width, height, channels, image, 100);
        else if (format == 2)
            stbi_write_png(filename_out, width, height, channels, image, 0);
